...
RUN /usr/local/go/bin/go get -tags=no_env github.com/dsandor/qt/cmd/...
...
COPY --from=base $GOPATH/src/github.com/dsandor/qt $GOPATH/src/github.com/dsandor/qt
COPY --from=dsandor/qt:js_base $HOME/emsdk $HOME/emsdk
COPY --from=dsandor/qt:js_base $HOME/.emscripten $HOME/.emscripten
COPY --from=dsandor/qt:js_base /usr/local/Qt-5.13.0 /usr/local/Qt-5.13.0
...
RUN cd $GOPATH/src/github.com/dsandor/qt/internal/examples/widgets/line_edits && $GOPATH/bin/qtdeploy build js && rm -rf ./deploy
